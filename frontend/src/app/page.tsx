...
export default function Home() {
  ...
  return (
    <div className="flex min-h-screen flex-col bg-[#08090d]">
            ...
            {(appState === 'IDLE' || appState === 'UPLOADING') && (
              <div>
                <div className="text-center mb-6">
                  <h1 className="font-mono text-lg sm:text-xl font-bold tracking-wider text-white uppercase">
                    SCAN YOUR FILE
                  </h1>
            ...
            {appState === 'ERROR' && (
              <div className="text-center pt-2">
                <button
                  type="button"
                  onClick={handleReset}
                  className="rounded-xs border border-white/[0.12] bg-white/[0.03] px-4 py-2 font-mono text-xs text-neutral-300 hover:text-white hover:border-white/[0.2] transition-colors cursor-pointer"
                >
                  [ Start Over ]
                </button>
              </div>
      ...
      {/* Minimal Footer */}
      <footer className="border-t border-white/[0.06] bg-[#08090d] py-5 text-center">
        <p className="font-mono text-[10px] sm:text-[11px] text-neutral-500 uppercase tracking-widest">
          SECUREX • REVIEW 1 PROTOTYPE
        </p>
      </footer>
    </div>
  );
